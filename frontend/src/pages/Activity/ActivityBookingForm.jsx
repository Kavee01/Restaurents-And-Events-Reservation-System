...
function ActivityBookingForm() {
  ...
  const checkBookedTimeSlots = async () => {
    try {
      ...
      const bookings = await sendRequest(
        `${import.meta.env.VITE_API_URL}/activitybooking/activity/${id}`,
        "GET"
      );
      ...
      dateBookings.forEach(booking => {
        ...
        const duration = 2; // Assuming 2 hours duration for activities
  ...
  const calculateTotal = () => {
    ...
    return activity.price * formData.participants;
  };
  ...
  const isTimeSlotAvailable = (timeSlot) => {
    ...
    const endHour = startHour + 2; // Assuming 2 hours duration for activities
    ...
    for (let i = 1; i < 4; i++) { // 2 hours = 4 half-hour slots
      const hour = Math.floor(startHour + (startMinute + i * 30) / 60);
      const minute = (startMinute + i * 30) % 60;
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.participants <= 0) {
      errorToast({
        title: "Error",
        message: "Please select at least 1 participant.",
      });
      return;
    }
    ...
    try {
      ...
      errorToast({
        title: "Error",
        message: `Failed to book activity: ${err.message}`,
      });
    } finally {
      setLoading(false);
    }
        ...
        <Container size="md">
          ...
          >
            ...
            <div style={{ position: 'relative', zIndex: 1 }}>
              <Title order={2} mb="xl" style={{ color: '#fff' }}>Book Activity</Title>
              ...
              <Grid>
                <Grid.Col md={7}>
                  ...
                  >
                    <Stack spacing="xs">
                      <Title order={3} style={{ color: '#d1ae36' }}>{activity.title}</Title>
                      ...
                      <Badge 
                        ...
                        mt="xs"
                      >
                        {activity.daysClosed && activity.daysClosed.length > 0 
                          ? `Closed on ${activity.daysClosed.map(day => day.replace(/[\[\]"]/g, '')).join(', ')}`
                          : 'Open Every Day'}
                      </Badge>
                    ...
                    <form onSubmit={handleSubmit}>
                      <Stack spacing="md">
                        ...
                        <NumberInput
                          ...
                          onChange={(value) => handleNumberChange("participants", value)}
                          min={1}
                          max={10}
                          required
                          styles={{
                        ...
                        {formData.date && formData.time && (
                          ...
                          >
                            <Stack spacing="xs">
                              ...
                              <Group position="apart">
                                <Text style={{ color: 'rgba(255, 255, 255, 0.7)' }} weight={500}>Participants:</Text>
                                <Text style={{ color: 'white' }}>{formData.participants}</Text>
                              </Group>
                              <Group position="apart">
                                <Text style={{ color: 'rgba(255, 255, 255, 0.7)' }} weight={500}>Price per person:</Text>
