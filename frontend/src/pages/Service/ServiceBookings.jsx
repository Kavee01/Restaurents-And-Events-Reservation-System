...
function ServiceBookings() {
  ...
  const fetchServiceBookings = async () => {
    ...
    try {
      ...
      errorToast({
        title: "Error",
        message: "Failed to load service bookings. Please try again."
      });
    } finally {
      setLoading(false);
    }
  ...
  const confirmCancelBooking = async () => {
    ...
    try {
      const endpoint = `${import.meta.env.VITE_API_URL}/servicebooking/${bookingToCancel._id}`;
      ...
      successToast({
        ...
        message: "Your service booking has been successfully canceled."
      });
    } catch (err) {
      console.log(err);
      close();
  ...
  const renderServiceBookingCard = (booking) => {
    const bookingDate = booking.date;
    const formattedDate = formatDateTime(bookingDate);
    const guests = booking.guests || 0;
    const location = booking.service?.location || 'N/A';
    const price = booking.totalPrice || 0;
    const title = booking.service?.title || 'Service Booking';
    const detailLink = `/services/${booking.service?._id}`;
        ...
        <Flex justify="space-between" align="flex-start">
          <Box>
            <Group position="apart" mb="xs">
              <Group spacing="sm">
                <IconCategory size={24} stroke={1.5} color="#d1ae36" />
                <Anchor 
                  component={Link} 
                  to={detailLink}
                  size="xl"
      ...
      <Stack spacing="md">
        <Text size="md">
          ...
          <Text span weight={600} color="#d1ae36">
            {bookingToCancel?.service?.title || "this service"}
          </Text>
          ?
        </Text>
            ...
            <div style={{ position: 'relative', zIndex: 1 }}>
              <Box mb="xl">
                <Group position="apart" align="center">
                  <Title order={2} style={{ color: '#fff' }}>My Service Bookings</Title>
                  <Button
                    component={Link}
                    to="/services"
                    style={{
      ...
      {loading ? (
        <LoadingSpinner />
                ...
                <SimpleGrid cols={1} spacing="lg">
                  {serviceBookings.map((booking) => renderServiceBookingCard(booking))}
                </SimpleGrid>
              ) : (
                <Box
                  p="xl"
